...
void TestSplitIntoWords() {
  ...
  for (size_t doc = 0u; doc < 5; doc++) {
    size_t line = 0u;
    for (auto word : SplitIntoWords(test_strings[doc])) {
      ASSERT_EQUAL(word, expected[doc][line++]);
    }
  }
}
...
chrono::milliseconds random_time() {
  random_device rd;
  mt19937_64 gen(rd());
  uniform_int_distribution<int64_t> dist(chrono::milliseconds::min().count(), chrono::milliseconds::min().count() + 2000);
...
void TestMapFind() {
  ...
  string query = "tell me why";
  for (auto word : SplitIntoWords(query)) {
    ASSERT_EQUAL(m[word], 1);
  }
}
